...
not available.");
  ...
  if (dist < 0.3 || pose.position.x >= -0.3)
  {
    ...
    // Report an artifact
    // Hardcoded to tunnel_circuit_practice_01's exginguisher_3 <-- This scores
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_EXTINGUISHER);
    // double art_x = 2.1;
    // double art_y = 37.0;
    // double art_z = 0.004;

    // backpack 1:      <-- This scores
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_BACKPACK);
    // double art_x = 41;
    // double art_y = 14.0;
    // double art_z = 0.004;

    // // tunnel practice 1 artifact origin in world coordinates:
    // double w_x = 10.0;
    // double w_y = 0.0;
    // double w_z = 0.0;

    // Artifacts from urban practice 1:
    // // vent 1:              <--- This doesn't score!!!
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_VENT);
    // double art_x = -68.15;
    // double art_y = -48;
    // double art_z = 3.0;   

    // //vent 2:               <-- This doesn't score!!!!
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_VENT);
    // double art_x = 56.0;
    // double art_y = -96.0;
    // double art_z = 19.85;  

    // //vent 3:                  <-- This doesn't score!!!
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_VENT);
    // double art_x = -68.0;
    // double art_y = -132.0;
    // double art_z = -7.0; 

    // // gas 2:                     <-- This doesn't score!!!
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_GAS);
    // double art_x = 640;
    // double art_y = 16.0;
    // double art_z = 11.0; 

    // gas 3:                     <-- This doesn't score!!!
    uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_GAS);
    double art_x = -128.000000;
    double art_y = -56.000000;
    double art_z = -9.000000; 
    ...
    // // phone 3:       <-- This scores
    // uint32_t arttype = static_cast<uint32_t>(subt::ArtifactType::TYPE_PHONE);
    // double art_x = -18;
    // double art_y = -116.0;
    // double art_z = -9;

    // urban practice 1 artifact origin in world coordinates:
    double w_x = 17.5;
    double w_y = 27;
    double w_z = 7.5;
    ...
    artifact.set_type(arttype);
    artifact.mutable_pose()->mutable_position()->set_x(art_x - w_x);
    artifact.mutable_pose()->mutable_position()->set_y(art_y - w_y);
    artifact.mutable_pose()->mutable_position()->set_z(art_z - w_z);

    
    //artifact.set_type(static_cast<uint32_t>(subt::ArtifactType::TYPE_VENT));
    //artifact.mutable_pose()->mutable_position()->set_x(-85.65);
    //artifact.mutable_pose()->mutable_position()->set_y(-75);
    //artifact.mutable_pose()->mutable_position()->set_z(-4.5);
